...
def validate_models():
    """Check required model files exist at startup"""
    missing = []
    for model_name in MODEL_NAMES:
        if model_name in MODEL_FILE_MAPPING:
            path = os.path.join(MODELS_DIR, MODEL_FILE_MAPPING[model_name])
            if not os.path.exists(path):
                missing.append(f"{model_name} ({path})")
    if missing:
        print("WARNING: Missing model files:", ", ".join(missing))
        print("Please download model checkpoints to the '/app/model' directory")
        print(
            "  - MatAnyone: https://github.com/pq-yang/MatAnyone/releases/download/v1.0.0/matanyone.pth"
        )
        print(
            "  - MatAnyone 2: https://github.com/pq-yang/MatAnyone2/releases/download/v1.0.0/matanyone2.pth"
        )
...
def load_model(model_name, device):
    ...
    # Unload if different model is cached
    for cached_name in list(_loaded_models.keys()):
        if cached_name != model_name:
            unload_model(cached_name)

    if model_name in _loaded_models:
        return _loaded_models[model_name]

    # Get checkpoint path from mapping
    if model_name not in MODEL_FILE_MAPPING:
        raise ValueError(f"Unknown model: {model_name}")

    ckpt_path = os.path.join(MODELS_DIR, MODEL_FILE_MAPPING[model_name])

    # Check file exists
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(f"Model checkpoint not found: {ckpt_path}")

    # Clear Hydra for clean state
    try:
        GlobalHydra.instance().clear()
    except:
        pass

    # Use same loader for both models (as in example)
    print(f"Loading model: {model_name} ({ckpt_path})...")
...
@torch.no_grad()
def process_with_reverse_tracking(
    video,
    projection,
    masks,
    crf=16,
    ...
    result_name = (
        file_name + "_" + str(projection_out).upper() + "_alpha_vr2ar" + file_extension
    )

    matanyone1 = load_model(model_selection, DEVICE_JOB)
    processor1 = InferenceCore(matanyone1, cfg=matanyone1.cfg, device=DEVICE_JOB)

    matanyone2 = load_model(model_selection, DEVICE_JOB)
    ...
    while ffmpeg.isOpen():
        ...
        if maskIdx < len(masks):
            ...
            if s1 > SSIM_THRESHOLD:
                s2 = ssim(
                    masks[maskIdx]["frameRGray"], cv2.cvtColor(imgR, cv2.COLOR_BGR2GRAY)
                )
                if not DEBUG:
                    print("ssim1", s1)
                print("ssim2", s2)
        ...
        if frame_match:
            ...
            if MASK_DEBUG:
                ...
                preview = Image.composite(
                    Image.new(
                        "RGB", (combined_mask.shape[1], combined_mask.shape[0]), "blue"
                    ),
                    Image.fromarray(img_scaled).convert("RGBA"),
                    mask.point(lambda p: 100 if p > 1 else 0),
                )
                preview.save(
                    "process/debug/match_" + str(current_frame).zfill(6) + ".png"
                )
            ...
            cv2.imwrite(
                "process/frames/" + str(current_frame).zfill(6) + ".png", img_scaled
            )
            combined_mask = np.zeros(
                (img_scaled.shape[0], img_scaled.shape[1]), dtype=np.uint8
            )
            cv2.imwrite(
                "process/masks/" + str(current_frame).zfill(6) + ".png", combined_mask
            )
            continue
        ...
        if MASK_DEBUG:
            ...
            preview.save(
                "process/debug/forward_" + str(current_frame).zfill(6) + ".png"
            )
        ...
        if reverse_track:
            ...
            frame_files = sorted(
                [
                    "process/frames/" + f
                    for f in os.listdir("process/frames/")
                    if f.endswith(".png")
                ],
                reverse=True,
            )
            ...
            for idx, frame_file in enumerate(frame_files):
                ...
                if erode:
                    mask_output_L_pha = cv2.erode(
                        mask_output_L_pha, (3, 3), iterations=2
                    )
                    mask_output_R_pha = cv2.erode(
                        mask_output_R_pha, (3, 3), iterations=2
                    )

                combined_mask = cv2.hconcat([mask_output_L_pha, mask_output_R_pha])
                maskA = cv2.imread(
                    frame_file.replace("frames", "masks"), cv2.IMREAD_UNCHANGED
                )
                ...
                if MASK_DEBUG:
                    ...
                    preview.save(
                        frame_file.replace("frames", "debug").replace(
                            ".png", "_rev.png"
                        )
                    )
...
def background_worker():
    ...
    while True:
        ...
        if surplus_url and not SURPLUS_IGNORE:
            try:
                start_job = "True" in str(
                    requests.get(surplus_url, verify=False).json()
                )
        ...
        if job["version"] == JOB_VERSION:
            ...
            try:
                ...
                result = process_with_reverse_tracking(
                    video_path,
                    job["projection"],
                    job["masks"],
                    job["crf"],
                    ...
                    if filebrowser_host := os.environ.get("FILEBROWSER_HOST"):
                        ...
                        try:
                            if filebrowser_user := os.environ.get("FILEBROWSER_USER"):
                                ...
                                client = FilebrowserClient(
                                    filebrowser_host, insecure=True
                                )
                            ...
                            couroutine = client.upload(
                                ...
                                remote_path=os.environ.get(
                                    "FILEBROWSER_PATH", os.path.basename(result)
                                ).replace("{filename}", os.path.basename(result)),
                                override=True,
                                concurrent=1,
                            )
...
def add_job(
    ...
    masksFiles = sorted(
        [
            f
            for f in os.listdir("masksL")
            if f.endswith(".png") and os.path.exists(os.path.join("masksR", f))
        ]
    )
    masks = []
    for f in masksFiles:
...
current_origin_frame = {"L": None, "R": None}
...
def add_mark(frame):
    ...
    for x, y, label in frame.point_set:
        cv2.drawMarker(
            ...
            markerSize=marker_final_size,
            thickness=marker_final_thickness,
        )
    return result
...
def generate_gallery_list():
    ...
    gallery_items = [
        (frame, str(str(max(0, int(idx * SECONDS - SECONDS / 2))) + " sec"))
...
def extract_frames(video, projection, mask_size, frames_seconds, keep_eq):
    ...
    if not keep_eq and str(projection) == "eq":
        ...
        os.system(
            f'ffmpeg -hide_banner -loglevel warning -hwaccel auto -i "{video.name}" -frames:v 1 -filter_complex "[0:v]{filter_complex}" -map "[v]" frames/0000.png'
        )
        if SECONDS > 0:
            os.system(
                f'ffmpeg -hide_banner -loglevel warning -hwaccel auto -i "{video.name}" -filter_complex "[0:v]fps=1/{SECONDS},{filter_complex}" -map "[v]" -start_number 1 frames/%04d.png'
            )
    else:
        os.system(
            f'ffmpeg -hide_banner -loglevel warning -hwaccel auto -i "{video.name}" -frames:v 1 -pix_fmt bgr24 frames/0000.png'
        )
        if SECONDS > 0:
            os.system(
                f'ffmpeg -hide_banner -loglevel warning -hwaccel auto -i "{video.name}" -vf fps=1/{SECONDS} -pix_fmt bgr24 -start_number 1 frames/%04d.png'
            )
...
def get_selected(selected):
    ...
    maskL = Image.fromarray(
        np.zeros([MASK_SIZE, MASK_SIZE, 3], dtype=np.uint8)
    ).convert("L")
    maskR = Image.fromarray(
        np.zeros([MASK_SIZE, MASK_SIZE, 3], dtype=np.uint8)
    ).convert("L")
...
def get_mask(
    ...
    if len(maskLPrompt) > 0:
        ...
        if len(maskLNegativePrompt) > 0:
            (_, negativeImgLMask) = sam2.predict(
                [frameL], maskLThreshold, maskLNegativePrompt
            )
            invNegativeMaskL = 255 - (
                negativeImgLMask[0].squeeze().cpu().numpy() * 255
            ).astype(np.uint8)
    ...
    if len(maskRPrompt) > 0:
        ...
        if len(maskRNegativePrompt) > 0:
            (_, negativeImgRMask) = sam2.predict(
                [frameR], maskRThreshold, maskRNegativePrompt
            )
            invNegativeMaskR = 255 - (
                negativeImgRMask[0].squeeze().cpu().numpy() * 255
            ).astype(np.uint8)
...
def merge_add_mask(maskL, maskR, mergedMaskL, mergedMaskR, dilate, erode):
    ...
    a, b, c, d = postprocess_mask(
        np.array(mergedMaskL), np.array(mergedMaskR), dilate, erode
    )
...
def generate_example(maskL, maskR, model_selection=DEFAULT_MODEL):
    frameL = current_origin_frame["L"].frame_data
    frameR = current_origin_frame["R"].frame_data
    ...
    return generate_mask_preview(cv2.imread("tmp_l.png"), "L"), generate_mask_preview(
        cv2.imread("tmp_r.png"), "R"
    )
...
with gr.Blocks() as demo:
    ...
    with gr.Column():
        ...
        gpu_dropdown = gr.Dropdown(
            choices=gpu_choices,
            label="Select GPU (currently only global for all jobs, do not change when a job is running!)",
            value=gpu_choices[gpu_select["idx"]],
        )
        gpu_dropdown.change(fn=on_gpu_change, inputs=gpu_dropdown, outputs=None)
        model_selection_dropdown = gr.Dropdown(
            choices=MODEL_NAMES,
            label="Select Matting Model",
            value="MatAnyone 2",
            info="MatAnyone 2 recommended for better quality",
        )
        gr.Markdown("## Stage 1 - Video")
        input_video = gr.File(label="Upload Video (MKV, MP4 or PKL job)")
        gr.Markdown("## Stage 2 - Video Parameter")
        projection_dropdown = gr.Dropdown(
            choices=["eq", "fisheye180", "fisheye190", "fisheye200"],
            label="VR Video Source Format",
            value="eq",
        )
        keep_eq = gr.Checkbox(
            label="Keep Equirectangular Format. Do not convert to fisheye view. (HereSphere VR Player can play equirectangular with packed alpha, but some artifacts appear at the 180° boundary)",
            value=False,
            info="",
        )
        mask_size = gr.Number(
        ...
        input_video.change(
            fn=input_video_changed, inputs=input_video, outputs=[mask_size, input_video]
        )
    ...
    with gr.Column():
        ...
        gr.Markdown("### Frames")
        gallery = gr.Gallery(
            label="Extracted Frames", show_label=True, columns=4, object_fit="contain"
        )
        select_button = gr.Button("Load Slected Projection Frame")
        with gr.Row():
            framePreviewL = gr.Image(
                value=None, type="numpy", format="png", image_mode="RGB"
            )
            framePreviewR = gr.Image(
                value=None, type="numpy", format="png", image_mode="RGB"
            )
    ...
    with gr.Tabs():
        with gr.Tab("Prompt"):
            with gr.Column():
                with gr.Row():
                    with gr.Column():
                        maskLPrompt = gr.Textbox(
                            label="Positive Prompt",
                            value="top person",
                            lines=1,
                            max_lines=1,
                        )
                        maskLNegativePrompt = gr.Textbox(
                            label="Negative Prompt", value="", lines=1, max_lines=1
                        )
                    ...
                    with gr.Column():
                        maskRPrompt = gr.Textbox(
                            label="Positive Prompt",
                            value="top person",
                            lines=1,
                            max_lines=1,
                        )
                        maskRNegativePrompt = gr.Textbox(
                            label="Negative Prompt", value="", lines=1, max_lines=1
                        )
        ...
        with gr.Tab("Points"):
            ...
            gr.Image.select(
                ...
                add_mark_point_l,
                inputs=[groupL],
                outputs=[maskSelectionL],
            )
            gr.Image.select(
                maskSelectionR,
                add_mark_point_r,
                inputs=[groupR],
                outputs=[maskSelectionR],
            )

            gr.Button.click(
                undoL, undo_last_point_l, inputs=None, outputs=maskSelectionL
            )
            gr.Button.click(
                undoR, undo_last_point_r, inputs=None, outputs=maskSelectionR
            )
            gr.Button.click(
                removeL, remove_all_points_l, inputs=None, outputs=maskSelectionL
            )
            gr.Button.click(
                removeR, remove_all_points_r, inputs=None, outputs=maskSelectionR
            )
    ...
    with gr.Column():
        ...
        gr.Markdown("Preview")
        with gr.Row():
            maskPreviewL = gr.Image(
                value=None, type="numpy", format="png", image_mode="RGB"
            )
            maskPreviewR = gr.Image(
                value=None, type="numpy", format="png", image_mode="RGB"
            )
        gr.Markdown("Mask")
        with gr.Row():
    ...
    with gr.Column():
        ...
        mask_dilate = gr.Slider(
            minimum=0, maximum=10, step=1, value=1, label="Dilate Iterrations"
        )
        mask_erode = gr.Slider(
            minimum=0, maximum=10, step=1, value=3, label="Erode Iterrations"
        )
        with gr.Row():
            postprocessedMaskL = gr.Image(
                value=None, type="numpy", format="png", image_mode="RGB"
            )
            postprocessedMaskR = gr.Image(
                value=None, type="numpy", format="png", image_mode="RGB"
            )

        gr.Markdown("Preview")
        previewMergedMask = gr.Image(
            value=None, type="numpy", format="png", image_mode="RGB"
        )

        gr.Markdown(
            "Optional: Generate MatAnyone Example Output (For debug purpose only)"
        )
        ...
        example_button.click(
            fn=generate_example,
            inputs=[postprocessedMaskL, postprocessedMaskR, model_selection_dropdown],
    ...
    with gr.Column():
        ...
        schedule_checkbox = gr.Checkbox(
            label="Enable Job Scheduling", value=SCHEDULE, info=""
        )
        ignore_surplus_checkbox = gr.Checkbox(
            label="Ignore Surplus Scheduling", value=SURPLUS_IGNORE, info=""
        )
        ...
        clear_completed_jobs_button.click(fn=clear_completed_jobs)

        schedule_checkbox.change(fn=set_schedule, inputs=schedule_checkbox)

        ignore_surplus_checkbox.change(
            fn=set_sureplus_ignore, inputs=ignore_surplus_checkbox
        )
